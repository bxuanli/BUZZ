...
# 更新内容：
    ...
    # 更改变道判定，临道车距离大于启动判定，不再限制一个车身长度
    # 更改避障减速判定，当自身车比侧向车大于10速时，启动减速适应
    # 修改避让判定，有横向距离才避让
    # 调整有一个车身长度就变道
    # 调整全减速不判定前方是否有车
    # 修改全减速，避免过度减速
...
class IDM():
    def __init__(self, a_bound=15, exv=40, t=1.2, a=3, b=2.4, gama=2, s0=5.0, s1=2.0): # V1.6 考虑exv值调大，不然加速度上不来
    ...
    def act(self, observation, luwang_list, time_test_start, traj=None):
                ...
                if traj:
                    if key != 'ego':    # 对于背景车
                        ...
                        try:
                            ...
                            sub_frame['v'] = abs((x_n-x)/(dt + 1e-6)) # 计算背景车速度
        ...
        # 取出车道数量值
        lane_num = self.lane_num(luwang_list)
        ...
        self.s_1 = self.s0 + v_1 * ((v_1 - fv_1) / (self.a_bound + 1e-6) ) + state[0, 4] # 期望前车距离
        self.s_2 = self.s0 + v_2 * ((fv_2 - v_2) / (self.a_bound + 1e-6) ) + state[0, 4] # 期望后车距离
        ...
        (np.absolute(y_gap_true) > 0.64))):
                ...
                a_idm = self.a * (1 - (v_1 / (self.exv + 1e-6)) ** self.gama + ((self.s_2 / (dis_gap_2+1e-6)) ** 2))
        ...
        and (dis_gap_1 > self.s_1 or dis_gap_1 == -1))) or (deside_avoid_acc == -1.1)): # 检测到避障减速指令或前向拥塞时，执行避障减速策略
            ...
            dis_gap_lateral_s_ = self.s0 + v_1 * (np.absolute(v_1 - state[a_dis_gap_lateral, 2]) / (self.a_bound + 1e-6)) + state[0, 4]
            a_idm = self.a * (1 - (v_1 / (self.exv + 1e-6)) ** self.gama - ((dis_gap_lateral_s_ / (dis_gap_lateral+1e-6)) ** 2)) # V1.8 调整减速度参数期望距离，提升减速性能
            if a_idm_acc < a_idm:
                a_idm = a_idm_acc
            ...
            a_idm = -20*self.a * (1 - (v_1 / (self.exv + 1e-6)) ** self.gama)

        if lane_center_dring_distance < 50 and a_idm >= 0: # 剩余可行使距离不到20减速
            # a_idm = -self.a * (1 - (v_1 / self.exv) ** self.gama)
            a_idm = self.a * (1 - (v_1 / (v_1 + 1e-6)) ** self.gama - ((50/ (lane_center_dring_distance+1e-6)) ** 2)) # V1.8 调整减速度参数期望距离，提升减速性能
            # a_idm = -15
        ...
        if (v_1 < 1  and a_idm < 0): # 速度低到1时不再减速，避免速度为0跳出测试
            a_idm = 0
        print('当前速度', v_1)
        print('当前加速度', a_idm)
    ...
    def deside_acc(self, state,lane_center, driving_orgin, lane_center_x_max, lane_center_x_min, lane_center_yaw_list, observation, final_goal_y, lane_now_yaw, exv_max_ego, lane_width):
        ...
        ((driving_orgin == True and (state[0, 0] - observation['test_setting']['goal']['x'][1]) < 40) or 
        (driving_orgin == False and ( observation['test_setting']['goal']['x'][1]) - state[0, 0] < 40))):
            self.s_1 = v_1 * ((v_1 - fv_1) / (self.a_bound + 1e-6) ) + self.s0 # 期望距离改为1米
            print('近终点与前车期望安全距离为：', self.s_1)
        else:
            self.s_1 = self.s0 + v_1 * ((v_1 - fv_1) / (self.a_bound + 1e-6)) + state[0, 4]
            print('与前车期望安全距离为：', self.s_1)
        # print(state)
        # 与前车相距100米开外、与前车相距20米开外、小于前车速度, 速度接近上限时匀速驾驶
        print('剩余可行驶距离',lane_center_dring_distance)
        # 前车速度大于本车/前方无车，剩余可行驶距离大于20，与前车速度差大于0.5
        if ((v_1 < fv_1 - 0.5 or dis_gap_1 == -1) and lane_center_dring_distance >= 20  and np.absolute(v_1 - fv_1) > 0.5) or dis_gap_1 > self.s_1 + 5 :
            if fv_1 > 30:
                self.exv = int(fv_1) + 10
            # 前车距离较大或者前方无车，
            if dis_gap_1 > self.s_1 or (dis_gap_1 == -1): # 与前车在期望距离外较大加速度，期望距离内控制加速度
                if dis_gap_1 != -1 and self.s_1 > 0: # 前方有车，小加速
                    a_idm = (dis_gap_1 / (self.s_1 + 1e-6)) * self.a * (1 - (v_1 / (self.exv + 1e-6)) ** self.gama - ((self.s_1 / (dis_gap_1 + 1e-6)) ** 2))
                    self.exv = exv_max_ego
                    print('当前加速度1',a_idm)
                else: # 前方没车，大加速
                    a_idm = 20*self.a * (1 - (v_1 / (self.exv + 1e-6)) ** self.gama) 
                ...
                a_idm = self.a * (1 - (v_1 / (self.exv + 1e-6)) ** self.gama - ((self.s_1 / (dis_gap_1 + 1e-6)) ** 2))
                self.exv = exv_max_ego
                print('当前加速度3',a_idm)

        elif (np.absolute(v_1 - fv_1) <= 0.5 and 2 < dis_gap_1 < 5):
            a_idm = 0
            print('当前匀速1')
        elif (v_1 - fv_2 > 0 and -1 < dis_gap_2 < 5):
            a_idm = 0
            print('当前匀速2')
        else:
            if dis_gap_1 != -1:
                a_idm = self.a * (1 - (v_1 / (self.exv + 1e-6)) ** self.gama - ((self.s_1 / (dis_gap_1 + 1e-6)) ** 2)) # V1.8 调整减速度参数期望距离，提升减速性能
        ...
        # 超出目标区域后，不再做剧烈加减速操作
        # if (driving_orgin and state[0, 0] < observation['test_setting']['goal']['x'][1] - 20) or driving_orgin == False and  state[0, 0] > observation['test_setting']['goal']['x'][1] + 20: 
        #     # print('ego车已驶出目标区域')
        #     if a_idm > 0:
        #         a_idm = 0
    ...
    def deside_yaw(self, observation, state, lane_center, lane_center_x_max, lane_center_x_min, lane_center_yaw_list, final_y_acc, lane_now_yaw, lane_width):
        ...
        # 增加针对目的区域的方向判断
        yaw_up, yaw_down, yaw_hold, y_gap_true = self.deside_ego_final(state, lane_now_yaw, final_goal_y, final_space, observation)
        y_gap_true = self.deside_ego_now_gap(state, lane_now_yaw, final_y, 0)
        # 增加针对目的区域的方向判断
        if driving_orgin:
            print("正向驾驶场景")
            # 判断是否落在目标中心线中心区域：在中心区域执行回正，在区域上方执行下偏，在区域下方执行上偏
            # if (state[0, 1]  >= (final_y - final_space)) and (state[0, 1]  <= (final_y + final_space)):
            if yaw_hold:
            ...
            if yaw_hold:
                ...
                if (final_y_acc_up == 1 or final_y_acc or (-2 < (y_gap_true) < -1)) and ((avoid_allow not in  [-10, 10.5, 20, -20, 1.1, -1.1]) or lane_center_dring_distance <= 100):  
                    ...
                    yaw = np.arctan((yaw_center - ego_yaw_now) * ego_l / (ego_v_now * ego_dt))
    ...
    def deside_change(self, state, lane_center, lane_center_x_max, lane_center_x_min, lane_center_yaw_list, lane_center_dring_distance, final_goal_y, observation, lane_now_yaw, lane_width): 
        ...
        for i in laneChange: # 在可行驶中心线中寻找目标
            ...
            if (np.absolute(y_gap_i) - state[0, 5]/2)  <= lane_width: # 判断 i中心线与本车纵向距离是否小于4米,确保是向相邻车道借道:
                ...
                for j in state[1:,1]: # 取出其他车 y值
                    ...
                    if (np.absolute(y_gap_i) - state[a, 5]/2) <= (lane_width / 2) :  # 判断 j车是否在 i中心道路上
                        # print(a,i,'车落在该道路上',(np.absolute(state[0, 0] - state[a, 0]) - (state[0, 4] + state[a, 4])/2), final_goal_y, state[0, 4])
                        ...
                        and (np.absolute(v_1 - state[a, 2]) / (self.a_bound + 1e-6) * (v_1 + 1e-6)) < (gap_ego_x))
                        or ((state[0, 0] < state[a, 0]) and (state[0, 2] < state[a, 2]) and 
                        (np.absolute(v_1 - state[a, 2]) / (self.a_bound + 1e-6) * (state[a, 2] + 1e-6)) < (gap_ego_x))
                        ...
                        and ((np.absolute(v_1 - state[a, 2]) / (self.a_bound + 1e-6) * (state[a, 2] + 1e-6)) < (gap_ego_x)))
                        or ((state[0, 0] < state[a, 0]) and (state[0, 2] <= state[a, 2]) and (gap_ego_x))
                        or ((state[0, 0] > state[a, 0]) and (state[0, 2] >= state[a, 2]) and (gap_ego_x)))))
                        ...
                        (np.absolute(state[a, 2] - state[0, 2]) * (lane_width / (state[0, 2] * 0.17365 + 1e-6)))))):
                            chang_allow_list.append(True)
                        # 如果剩余距离不足100，放宽变道条件，以兼容汇入汇出场景
                            ...
                            # print(a,'车与本车横向',gap_ego_x, i)
                            # print(final_goal_y < i < state[0, 1],(np.absolute(state[a, 2] - state[0, 2]) * (lane_width / (state[0, 2] * 0.17365 + 1e-6))))
                            if (gap_ego_x) > 0: # 与本车至少有5米横向距离
                                if driving_orgin: # 正向驾驶场景
                                    if state[0, 0] > state[a, 0]: # a车在本车前方
                                        if state[0, 2] <= state[a, 2]: # 前车速度大于等于本车
                                            ...
                                            if (np.absolute(state[0, 2] - state[a, 2]) / (self.a_bound + 1e-6) * (state[0, 2] + 1e-6)) < (gap_ego_x):
                                                chang_allow_list.append(True)
                                            else:
                                                chang_allow_list.append(False)
                                                # print(a,'在本车前方邻车道7米内')
            ...
            if False not in chang_allow_list: 
                final_y_acc = i
                ...
                if lane_center_dring_distance > 121:
                    ...
                    if np.absolute(y_gap_now) > 0.2:
                        ...
                        if (y_gap_now < 0) or (y_gap_now + state[0, 5]/2 < 1 and y_gap_now > 0): # 车在终点线下方或者车上边界不超过终点线上方1米
                            final_y_acc_up = 1
                            # print(y_gap_now , y_gap_now + state[0, 5]/2, '上变道条件检测')
        ...
        if final_y_acc:
            ...
            print("无可变道中心线")
        # 如果在终点线中心点附近，可微调车身
        # if np.abs(state[0, 1] - final_goal_y) < 2.5:
        #     final_y_acc_up = 1
        #     final_y_acc_down = 1
    ...
    def deside_avoid(self, state, final_y, final_y_acc_up, final_y_acc_down, lane_center_dring_distance, final_goal_y, observation, change_refuse_list, lane_now_yaw):
        ...
        (driving_orgin == False and (0 < observation['test_setting']['goal']['x'][1]) - state[0, 0] < 100))):
            ...
            self.s_1 = self.s0 + v_1 * ((v_1 - fv_1) / (self.a_bound + 1e-6) ) + state[0, 4] # 期望前车距离
        self.s_2 = self.s0 + v_2 * ((v_2 - fv_2) / (self.a_bound + 1e-6) ) + state[0, 4] # 期望后车距离
        avoid_allow = 0
        deside_avoid_acc = 50
        # exv_up = 0
        a = 0
            ...
            if gap_ego_x < 50: # 检测 i车辆与本车横向距离 
                ...
                # 纵向距离判断更改为
                    ...
                    and (np.absolute(state[0, 0] - observation['test_setting']['goal']['x'][1]) - (gap_ego_x)) > 10):
                        ...
                        lateral_avoid_allow = (gap_ego_x > 0) or ((gap_ego_x < 0) and (gap_ego_y > (state[0, 4]  * np.sin(0.174533))))  # 横向间距大于0， 或者纵向间距大于车身偏移量，才可执行避让
                        # lateral_avoid_allow = (gap_ego_x > 0)
                        ...
                        (a_lateral_avoidance_down == False or (np.absolute(state[0, 0] - state[a, 0]) < a_lateral_avoidance_down))): 
                            ...
                            and (np.absolute(v_1 - state[a, 2]) < 10)))) and lateral_avoid_allow):     
                                ...
                                and state[a, 2] - state[0, 2] < 5): # 在本车前方
                                    print(a, '在本车前方')
                                    if ((gap_ego_x < 0 
                                    and ( ( 10 > state[a, 2] - state[0, 2] > 0 ))) and (-1 not in a_clamp_avoidance)):
                                        print('无上避让条件，全减速通过')
                                        avoid_allow = 10
                                        ...
                                        elif ((np.absolute(v_1 - state[a, 2]) / (self.a_bound + 1e-6) * (v_1 + 1e-6)) < (gap_ego_x)
                                    ...
                                    if ((gap_ego_x < 0 
                                    and (( 8 > state[a, 2] - state[0, 2] > 0 ))) and (-1 not in a_clamp_avoidance)):
                                        print('无上避让条件，全减速通过')
                                        avoid_allow = 10
                                        deside_avoid_acc = -1.2 # -1.2代表全减速通过
                                    else:
                                        if gap_ego_x < 0 and (dis_gap_1 > self.s_1 or dis_gap_1 == -1) and (state[0, 2] > state[a, 2]):
                                            ...
                                            avoid_allow = 10
                                            deside_avoid_acc = 1.2
                                        elif ((np.absolute(v_1 - state[a, 2]) / (self.a_bound + 1e-6) * (state[a, 2] + 1e-6)) < (gap_ego_x)
                        ...
                        (a_lateral_avoidance_up == False or (np.absolute(state[0, 0] - state[a, 0]) < a_lateral_avoidance_up))):      
                            ...
                            # 终点在下方
                                ...
                                if ((state[0, 0] > state[a, 0]) and (driving_orgin == True)) or ((state[0, 0] < state[a, 0]) and (driving_orgin == False)): # 在本车前方
                                    ...
                                    # 如果有前向碰撞风险，全减速通过
                                    if ((((gap_ego_x < 0) 
                                    and (( 10 > state[a, 2] - state[0, 2] > 0 ) or (dis_gap_1 - self.s_1 < 3 * state[0, 4])))) and (-1 not in a_clamp_avoidance)):
                                        print('无下避让条件，全减速通过')
                                        avoid_allow = -10
                                        deside_avoid_acc = -1.2 # -1.2代表全减速通过
                                    else:
                                        ...
                                        and (self.s_2 + 5 < dis_gap_2 or dis_gap_2 == -1) and (state[a, 2] - state[0, 2] < 10)) or (state[0, 2] - state[a, 2] > 10)):     
                                            print('无下避让条件，减速通过')
                                            avoid_allow = -10
                                            deside_avoid_acc = -1
                                        elif (((np.absolute(v_1 - state[a, 2]) / (self.a_bound + 1e-6) * (v_1 + 1e-6)) < gap_ego_x
                                    ...
                                    and (( 10 > state[a, 2] - state[0, 2] > 0 ))) and (-1 not in a_clamp_avoidance)):
                                        ...
                                        if (gap_ego_x) < 0 and (dis_gap_1 > self.s_1 or dis_gap_1 == -1) and (state[a, 2] < state[0, 2]):
                                            print('无下避让条件，全加速通过')
                                            avoid_allow = -10
                                            deside_avoid_acc = 1.2
                                        elif (np.absolute(v_1 - state[a, 2]) / (self.a_bound + 1e-6) * (state[a, 2]  + 1e-6)) < (dis_gap_1):
                                            print('无下避让条件，加速通过')
                                            avoid_allow = -10
                                            deside_avoid_acc = 1
                                        elif state[a, 2] - state[0, 2] >= 5 and (self.s_1 < dis_gap_1 or dis_gap_1 == -1):
            ...
            if (gap_ego_x < 70) and (avoid_allow != (1.1 and -1.1)): # 前后向防撞检测7米，侧向检测7米 
                ...
                if 0 >= (gap_ego_y): # 检测 i车辆与本车纵向距离，此处表明为前后跟车
                    ...
                    if driving_orgin: # 正向驾驶场景
                        ...
                        ((((np.absolute(state[0, 2] - state[a, 2]) / (self.a_bound + 1e-6) * (state[a, 2] + 1e-6)) > (gap_ego_x))
                        or ((np.absolute(state[0, 2] - state[a, 2]) / (self.a_bound + 1e-6) * (state[0, 2] + 1e-6)) > dis_gap_1))
                        or ((gap_ego_x) < 1)))
                        ...
                        and (np.absolute(state[0, 0] - observation['test_setting']['goal']['x'][1]) - ((gap_ego_x))) > 10)): # 后向碰撞检测
                            ...
                            if np.absolute(final_goal_y - state[0, 1]) > 0.6 : 
                                # 终点在上方
                                if y_gap_true < -0.6 and final_y_acc_up == 1:
                        ...
                        ((np.absolute(state[0, 2] - state[a, 2]) / (self.a_bound + 1e-6) * (state[0, 2] + 1e-6)) > (gap_ego_x)
                        or ((np.absolute(state[0, 2] - state[a, 2]) / (self.a_bound + 1e-6) * (fv_2 + 1e-6)) > dis_gap_2)))
                        ...
                        or ((gap_ego_x) < 1)))
                        and (a_lateral_avoidance_after == False or (np.absolute(state[0, 0] - state[a, 0]) < a_lateral_avoidance_after))
                        and (np.absolute(state[0, 0] - observation['test_setting']['goal']['x'][1]) - ((gap_ego_x))) > 10)and dis_gap_2 < 15): # 后向碰撞检测
                            a_lateral_avoidance_after = np.absolute(state[0, 0] - state[a, 0]) 
                            a_clamp_avoidance.append(-1)  # 前后碰撞参数记录
                            print(a, '与本车有后向碰撞风险')
                            # 如果本车已在终点线车道，只做减速处理，提升终点通过率
    ...
    # 车道数量输出方法
    def lane_num(self, lane_info_list):
        # 车道数量初始为0
        lane_num = 0
        # 检测 list中元素数量 
        for i in lane_info_list:
            lane_num += 1
        return int(lane_num/4)
    ...
    def deside_ego_final(self, state, lane_now_yaw, final_y, final_space, observation):
        ...
        final_dy = final_space / (np.cos(lane_now_yaw) + 1e-6)
        y_final = np.tan(lane_now_yaw) * x_final + b
        if y_final + final_dy < final_y:
            yaw_up = True # 终点在下方
            # print(y_final,final_dy,final_y )
